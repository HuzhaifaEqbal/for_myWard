...
import DuolingoPrayerWidget from '@/components/prayer/DuolingoPrayerWidget';
import MeccaSyncRadar from '@/components/prayer/MeccaSyncRadar';
import AdhanAudioHub from '@/components/prayer/AdhanAudioHub';
...
// Clean SVG Icons for Main Page Navigation & Headers (Strictly Zero Emojis)
const TabSVGs = {
  Quran: () => (
...
export default function Home() {
  ...
  const [worshipTab, setWorshipTab] = useState<'quran' | 'tasbih' | 'prayers' | 'radar' | 'audio' | 'virtues'>('quran');
    ...
    return (
      <div className="min-h-screen bg-background text-on-surface flex items-center justify-center font-ui-header">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-sm text-primary">جاري استحضار لوحة الأبدية الروحانية...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <main className="min-h-screen p-void-md md:p-void-xl pt-20 md:pt-24 flex flex-col gap-void-xl max-w-[1440px] mx-auto select-none" suppressHydrationWarning>
      ...
      {/* 0. Sticky Floating Top Bar */}
      <StickyPrayerHUD onOpenPrayerSection={() => {
        setWorshipTab('prayers');
        scrollToWorshipSection();
      }} />

      {/* 0.1 Duolingo-Style Floating Interactive Prayer Widget */}
      <DuolingoPrayerWidget onOpenFullSchedule={() => {
        setWorshipTab('radar');
        scrollToWorshipSection();
      }} />
      ...
      <header className="flex flex-col gap-4 text-center md:text-right">
        ...
        <p className="font-body-main text-on-surface-variant max-w-3xl text-base md:text-lg">
          منظومتك الشاملة للتأمل في عظمة الخالق من أدق الذرات إلى أطراف الكون، توثيق وردك اليومي، إهداء ثواب الطاعات لمن تحب، رادار مكة المكرمة، وأصوات كبار المؤذنين.
        </p>
      </header>
      ...
      {/* 4. Smart Worship Hub (Quran, Tasbih, Prayers, Mecca Radar, Adhan Voices, Virtues) */}
      <section id="worship-section" className="flex flex-col gap-void-md">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-ui-header text-2xl text-on-surface font-bold">منظومة العبادات ومواقيت الصلاة</h2>
              <p className="text-xs text-on-surface-variant">أعمالك الموثقة تُخصم من يوم القيامة وتغرس نباتات حقيقية في واحتك</p>
            </div>
          </div>

          {/* Worship Navigation Tabs */}
          <div className="flex items-center bg-surface-container-high/60 p-1 rounded-2xl border border-outline-variant/30 backdrop-blur-md overflow-x-auto max-w-full">
            <button
              ...
              className={`px-3 py-2 rounded-xl text-xs md:text-sm font-ui-header transition-all flex items-center gap-1.5 whitespace-nowrap ${
                worshipTab === 'quran'
            ...
            <button
              onClick={() => setWorshipTab('radar')}
              className={`px-3 py-2 rounded-xl text-xs md:text-sm font-ui-header transition-all flex items-center gap-1.5 whitespace-nowrap ${
                worshipTab === 'radar'
                  ? 'bg-cyan-600 text-white font-bold shadow-[0_0_15px_rgba(8,145,178,0.3)]'
                  : 'text-on-surface-variant hover:text-on-surface'
              }`}
            >
              <TabSVGs.Radar />
            ...
            <button
              onClick={() => setWorshipTab('audio')}
              className={`px-3 py-2 rounded-xl text-xs md:text-sm font-ui-header transition-all flex items-center gap-1.5 whitespace-nowrap ${
                worshipTab === 'audio'
                  ? 'bg-indigo-600 text-white font-bold shadow-[0_0_15px_rgba(79,70,229,0.3)]'
                  : 'text-on-surface-variant hover:text-on-surface'
              }`}
            >
              <TabSVGs.Audio />
        ...
        <div className="w-full">
          ...
          {worshipTab === 'radar' && <MeccaSyncRadar />}
          {worshipTab === 'audio' && <AdhanAudioHub />}
