...
export default function LogarithmicUniverseEngine() {
  ...
  // Preload and cache all 10 images eagerly
  useEffect(() => {
    ...
    const render = () => {
      ...
      universeData.forEach((item) => {
        ...
        if (screenSize >= 8 && screenSize <= width * 3.5) {
          ...
          // Draw the actual image directly
          if (img) {
            try {
              // Circular background badge
              ctx.beginPath();
              ctx.arc(centerX, centerY, drawRadius, 0, Math.PI * 2);
              ctx.fillStyle = 'rgba(18, 33, 49, 0.85)';
              ctx.fill();

              // Draw image centered inside
              ctx.drawImage(
                img,
                centerX - drawRadius,
                centerY - drawRadius,
          ...
          // Glowing border
          ctx.strokeStyle = 'rgba(247, 190, 29, 0.5)';
          ctx.lineWidth = Math.max(1.5, Math.min(4, screenSize * 0.03));
          ctx.beginPath();
          ctx.arc(centerX, centerY, drawRadius, 0, Math.PI * 2);
          ctx.stroke();

          // Draw Object Name & Scale below the item
          if (screenSize >= 26 && screenSize <= width * 1.8) {
            ctx.font = 'bold 15px Geist, sans-serif';
            ctx.textAlign = 'center';
            ctx.textBaseline = 'top';

            const textY = Math.min(height - 42, centerY + drawRadius + 14);

            // Text background pill
            const text = item.name_ar;
            const textWidth = ctx.measureText(text).width;
            ctx.fillStyle = 'rgba(5, 20, 36, 0.85)';
            ctx.beginPath();
      ...
      ctx.fillStyle = 'rgba(168, 180, 208, 0.85)';
              ...
              <div className="flex items-center justify-between border-b border-white/10 pb-3 mb-4">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-xl bg-surface p-1 border border-tertiary/40 flex items-center justify-center">
                    <img src={activeItem.image_url} alt="" className="w-full h-full object-contain rounded-lg" />
                  </div>
                  <div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
          {universeData.map((item) => {
            ...
            return (
              ...
              >
                <img src={item.image_url} alt="" className="w-3.5 h-3.5 object-contain rounded-full" />
